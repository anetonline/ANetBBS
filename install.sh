...
# Real gap found in a security audit: all three of these used to
# build a string like `eval "$var_name=\"\${input:-$default}\""` and
# eval it -- $input is raw terminal text (BBS Name, Description,
# Sysop username, Domain, etc. all go through ask()), so a value
# containing a double-quote, backtick, or $(...) broke out of the
# assignment and executed as shell code, running as root since this
# script runs via sudo. `printf -v` is bash's built-in safe indirect
# assignment -- it assigns the literal value to the named variable
# without ever treating it as shell code, no eval needed.
ask() {
...
# created by the plain `cat >` below, landing at the default umask
# (typically 644) for the entire time this heredoc takes to write
# (SECRET_KEY and other values are still being substituted line by
# line) until the chmod 600 that used to run only at the very end of
# this block -- a real window during which the SECRET_KEY sat
# world-readable. Same fix philosophy as update.sh's backup-dir
# lock-at-creation fix below: born at the right mode via umask,
# instead of narrowed after the fact.
(
umask 177
cat > "$ENV_FILE" << ENVEOF
...
# password hashes, session tokens, and private messages -- 644 in a
# 755 dir means any local unprivileged user on the box could read it
# outright. No process anywhere in this codebase needs group/world
# read access to it (the app runs as SERVICE_USER; every deploy/backup/
# restore script that touches it runs as root via sudo, which bypasses
# file permissions entirely) -- 644 was never a deliberate choice,
# just whatever the file happened to land at. See deploy/run_restore.sh
# for the matching fix on the restore-from-backup path.
chmod 600 "$INSTALL_DIR/data"/*.db 2>/dev/null || true
