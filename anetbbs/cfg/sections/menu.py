"""BBS Menu editor section (anetbbs-cfg) -- telnet/SSH/rlogin terminal
menus. Two-level list like the Echomail section: pick a menu, drill into
its items. `ansi_screen` (raw ANSI art banner) is web-admin-only, same
"known gap" pattern as Board.ansi_banner.
"""
...
MENU_FIELDS = [
    ...
    {"key": "is_default", "label": "Default Menu (shown after login)", "kind": "bool"},
    {"key": "min_access", "label": "Min Access Level", "kind": "int"},
]

MENU_HELP = ["ANSI banner screen editing: web admin only (Admin -> Menus)."]
...
ITEM_COLUMNS = [
    ...
    ("Action", 14, lambda i: i.action_type),
...
def list_menus():
    return BbsMenu.query.order_by(BbsMenu.name).all()
...
def list_items(menu):
    return BbsMenuItem.query.filter_by(menu_id=menu.id).order_by(BbsMenuItem.sort_order).all()
...
def create_item(menu, data):
    i = BbsMenuItem(menu_id=menu.id, **data)
    db.session.add(i)
    db.session.commit()
    return i
...
def _add_menu(stdscr):
    data = ui.run_form(stdscr, "New Menu", MENU_FIELDS, dict(MENU_NEW_DEFAULTS), help_lines=MENU_HELP)
    if data is None:
        return
...
def _edit_menu_settings(stdscr, m):
    data = ui.run_form(stdscr, f"Menu Settings: {m.name}", MENU_FIELDS, values_from_menu(m), help_lines=MENU_HELP)
    if data is None:
        return
    try:
        update_menu(m, data)
...
def _delete_menu(stdscr, m):
    item_count = BbsMenuItem.query.filter_by(menu_id=m.id).count()
...
def _add_item(menu):
    def _cb(stdscr):
        data = ui.run_form(stdscr, f"New Item in {menu.name}", ITEM_FIELDS, dict(ITEM_NEW_DEFAULTS), help_lines=ITEM_HELP)
        if data is None:
            return
...
def _edit_item(stdscr, i):
    data = ui.run_form(stdscr, f"Edit Item: {i.label}", ITEM_FIELDS, values_from_item(i), help_lines=ITEM_HELP)
    if data is None:
        return
    update_item(i, data)
...
def _manage_items(stdscr, m):
    ...
    ui.run_list(
        stdscr, f"Menu Items: {m.name}", ITEM_COLUMNS, lambda: list_items(m),
...
def run(stdscr):
    ui.run_list(
        stdscr, "BBS Menus", MENU_COLUMNS, list_menus,
