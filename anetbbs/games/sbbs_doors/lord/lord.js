	...
	if (state.married_to_violet > -1) {
		...
		if (player.seen_violet) {
			...
			if (state.married_to_violet === player.Record) {
				...
				// ANetBBS patch: defend against a stale married_to_violet that
				// points at a record that's been deleted (or never existed).
				// Upstream LORD assumes player_get always succeeds here and
				// crashes with `Cannot read property 'name' of null`. Reset
				// the flag and fall through to a generic story beat instead
				// of the personalised "you curse <name>" line.
				if (op === null) {
					state.married_to_violet = -1;
					try { put_state(); } catch (_e) {}
					sln('  News reaches you that Violet has been spotted with a new');
					sln('  suitor — but no one remembers his name.  Grizelda kisses');
					lln('  you painfully as you scream in horror.');
				} else {
...
function check_gameover()
{
	...
	if (state.won_by >= 0) {
		...
		// ANetBBS patch: if state.won_by points at a record that no
		// longer exists (deleted player, mid-tournament reset, or
		// any state-file inconsistency), upstream LORD crashed on
		// `wb.name`. Reset the flag, save state, and let the next
		// launch start the game fresh instead of looping on a
		// permanent "game over" screen.
		if (wb === null) {
			state.won_by = -1;
			try { put_state(); } catch (_e) {}
			sln('');
			lln('  `2The legends of the great warrior are fading from memory.');
			sln('  The Red Dragon stirs once more...');
			sln('');
			return;
		}
		load_player(false);
...
function start() {
	...
	// ANetBBS patch: RIP probe disabled. The upstream probe sends an
	// \x1b[!\x1b[6n sequence and blocks read_str(10000, ...) for up
	// to 10 seconds waiting for a RIPSCRIP terminal response. No
	// browser-side or modern terminal client we care about (xterm.js,
	// SyncTERM, NetRunner, mTelnet) advertises RIPSCRIP, so the probe
	// just adds a hard 10-second stall at door entry. Skip it; leave
	// `rip = false` (the var default from the top of this file).
	// To re-enable: revert this hunk.
	/*
