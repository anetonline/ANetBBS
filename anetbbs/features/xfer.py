...
# But the socket is a telnet connection: 0xFF is IAC, and a telnet client
...
# the ZMODEM stream is corrupt) and re-inserted on the way out. The old
# bridge passed bytes through raw, so a compliant client's doubled IAC
# reached rz as a spurious extra 0xFF and corrupted the transfer; only a
# client doing no telnet processing (a plain raw socket) happened to work.
_IAC = 0xFF
_SB, _SE = 0xFA, 0xF0
...
async def send_file(session, filepath: str, protocol: str = 'zmodem') -> bool:
    ...
    # Ask the client for 8-bit-clean mode before the transfer. This is
    # best-effort; the escape/unescape codec above is what actually
    # guarantees byte-transparency, not the client's BINARY agreement.
    await _negotiate_binary(session, True)

    transfer_done = asyncio.Event()

    async def _proc_to_session():
        """Forward sz stdout → writer, doubling IAC for the telnet peer."""
        try:
            while True:
                chunk = await proc.stdout.read(4096)
                if not chunk:
                    break
                session.writer.write(_telnet_escape(chunk))
                await session.writer.drain()
