...
Box is exactly 79 chars wide (╔ + 77═ + ╗) to fit an 80-column terminal.
Max 4 posts per page to stay within 23 lines.
...
# Box is 79 chars wide: ╔ + 77═ + ╗  (WALL_WIDTH = inner content = 77)
WALL_WIDTH = 77
...
def _header(page: int, total: int, border: str, title_c: str) -> bytes:
    W = WALL_WIDTH  # 77 — box total = ╔+77═+╗ = 79 chars
    hbar = '═' * W
...
def _footer(is_admin: bool, has_older: bool, has_newer: bool,
            border: str, key_c: str) -> bytes:
    W = WALL_WIDTH
    hbar = '═' * W
    opts = []
    if has_newer:
        opts.append(f'{_BOLD}{key_c}[N]{_RST}ewer')
    if has_older:
        opts.append(f'{_BOLD}{key_c}[O]{_RST}lder')
    opts.append(f'{_BOLD}{key_c}[W]{_RST}rite')
    if is_admin:
        opts.append(f'{_BOLD}{_RD}[D]{_RST}el')
...
def _render_post(post, is_admin: bool, user_c: str) -> bytes:
...
async def show_wall(session, allow_post: bool = True) -> None:
    ...
    while True:
        ...
        session.writer.write(_header(page, total, border, title_c))
        if not posts:
            session.writer.write(
                _enc(f'\r\n  {_DIM}No posts yet \x14 be the first to write on the wall!{_RST}\r\n\r\n'))
        else:
            for post in posts:
                session.writer.write(_render_post(post, is_admin, user_c))
        session.writer.write(_footer(is_admin, has_older, has_newer, border, key_c))
        ...
        if choice in ('q', ''):
            ...
            await _post_to_wall(session, border, key_c)
...
async def _post_to_wall(session, border: str, key_c: str) -> None:
    from ..features.bbs_ui import _app
    ...
    session.writer.write(_enc(
        ...
        f'{_DIM}Max 2 lines x {WALL_WIDTH} chars. Leave blank to cancel.{_RST}\r\n\r\n'
    ))
    await session.writer.drain()
    ...
    raw1 = _truncate_pipes(raw1, WALL_WIDTH)

    raw2 = (await session.read_line(f'{key_c}Line 2 (Enter to skip):{_RST} ') or '').rstrip('\r\n')
    raw2 = _truncate_pipes(raw2, WALL_WIDTH) if raw2.strip() else None
