...
Fixed a bug where a session idle for an extended period over SSH could
go permanently unresponsive if the underlying network connection
silently dropped, with no way to recover except a server restart.
...
A fifth audit pass was carried out, focused specifically on the web
interface and the terminal (telnet/SSH/rlogin) client this time, using
the same severity-tiered approach as the four prior rounds (v1.0.38,
v1.0.39, v1.0.48, v1.0.66). As with those rounds, this entry
intentionally omits vulnerability specifics in the interest of
responsible disclosure; sysops running an older release should update
at their earliest convenience. This was the broadest single pass yet —
around 27 real, independently-verified findings, every one with a
dedicated regression test, including a second independent verification
pass over the highest-stakes fixes after the main sweep. In summary,
this pass:

- Closed several real access-control gaps, including a case where one
  user's live, in-progress door-game session could be manipulated by
  another logged-in user, a case where private mail was reachable
  through an unauthenticated public export path, and a case where a
  downstream network node could receive content scoped to a different
  hub identity than its own.
- Closed a recurring class of gap, found and fixed independently
  across roughly eight different subsystems (terminal chat, message
  composition and display, an internal network bridge, file/game-data
  import, and more), where remotely- or peer-supplied text could
  manipulate a caller's own terminal display in ways it shouldn't be
  able to.
- Closed a stored cross-site-scripting gap in a self-published content
  feature, and hardened file-upload integrity checking against a
  resource-exhaustion technique.
- Fixed two unauthenticated-connection resource-exhaustion gaps in the
  terminal server layer.
- Extended an existing self-protection guard (an admin action that
  already refused to let a sysop lock themselves out via the web
  panel) to the equivalent terminal-side admin screen, which was
  missing it.
- Fixed several real correctness bugs found along the way: a broken
  constructor call that silently killed a welcome-message feature and
  a bulk-messaging feature, a command-parsing bug in the sysop
  console, a menu dispatch path that could end a caller's whole
  session on one bad navigation, a resource-cleanup gap that left
  files accumulating on disk indefinitely in two unrelated features,
  and a separate gap where a hung external process spawned by a
  configurable feature wasn't being fully terminated on timeout.
- Removed a small amount of confirmed-dead code and corrected several
  stale documentation pages (outdated version references, missing
  coverage of recently-added features, and a couple of long-standing
  example/placeholder values that named a real person rather than a
  generic example).

New regression tests were added throughout — every fix in this pass
has one, verified against the pre-fix code before being counted as
done. A few lower-priority items were deliberately left as open
questions for a sysop's own judgment call rather than unilaterally
