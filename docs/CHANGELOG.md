...
Phase 4 (the final phase) of the 4-part audit list (auth/session core, file areas, message boards, install/update re-verify). Two parallel research passes (`install.sh`'s fresh-install path, `update.sh` + `anetbbs/installer/`'s upgrade path), every finding personally verified against the real code before fixing. Also includes a follow-up fix Jerry asked for after seeing it listed as deferred in v1.0b2.221: webhook board scoping.
