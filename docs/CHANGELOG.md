...
- NEW TOOL: `tools/post_tech_news.py` — a standalone, cron-invoked script that posts the top daily stories from an RSS feed into `ANN.TECH` (both the BinkP and QWK area rows), with dedup by story link so re-running doesn't repost. Not a general feature — ships with the codebase but does nothing automatically; intended for bbs.a-net.fyi's own crontab specifically. 3 tests in `tests/test_post_tech_news.py`, mocking the feed so nothing needs real network access to verify.
