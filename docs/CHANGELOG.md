...
- FIX (live-caught): a sysop activating the seeded "ANotherNetwork"/"ANotherNetwork (QWK)" network rows *on the hub itself* (bbs.a-net.fyi) causes the poller to try dialing out to its own address every 60 seconds, failing both BinkP auth and QWK download since those rows are the *peer-side* config for connecting *to* the hub, not for the hub's own participation. No code change (working as designed once understood) — documented in the wiki/docs follow-up: the hub's own local users read/post in the `ANN.*` areas directly; only peer installs should activate those two network rows.
