...
- NEW TOOL: `tools/post_tech_news.py` — a bbs.a-net.fyi-specific standalone script (not a general feature; ships inert, only runs if you set up your own crontab entry for it) that posts the top daily stories from an RSS feed into `ANN.TECH` (both the BinkP and QWK area rows), deduped by story link. Verified end-to-end against the real Ars Technica feed (dry-run, then a real post, then a second run confirming no duplicates), plus 3 automated tests with a mocked feed.
