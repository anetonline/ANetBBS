...
- Live-caught during setup, not a bug: activating the seeded "ANotherNetwork"/"ANotherNetwork (QWK)" rows *on the hub itself* makes the poller try to dial its own address every 60 seconds — those rows are the peer-side config for connecting *to* the hub, not for the hub's own participation. The hub's own users read/post in the `ANN.*` areas directly as local message areas; only peer installs activate those two network rows.
