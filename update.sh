...
if [[ -f "$SUDOERS_SRC" ]]; then
    if [[ -z "$SERVICE_USER" ]]; then
        ...
        # ... > file` redirection above creates $SUDOERS_DST.tmp at the
        # ambient umask (typically 644, world-readable) -- it then sits
        # under /etc/sudoers.d/ at that mode through the visudo
        # validation AND the mv below, only getting locked to 0440 by
        # the chmod that already runs after the mv. Same race-window
        # class already fixed elsewhere in this script for .env (see
        # its own "umask" comment) -- lock it down immediately at
        # creation instead. visudo -cf only needs to read the file,
        # which root can always do.
