"""Regression test for the bundled "Uber Blox" door seed
(anetbbs/web_app.py, BUNDLED_DOORS list) -- the 16th door wired up to
...
Real Matt Johnson (mcmlxxix) door (a block-clearing puzzle game, like
GameHouse's "Super Collapse" -- NOT a Tetris clone; an earlier,
unverified memory note calling it "redundant with Synchronetris" was
wrong, corrected during this batch's own triage pass). Same author as
Fat Fish/Dice Warz ][/Maze Race/Star Stocks, source unmodified, bundled
at anetbbs/games/sbbs_doors/uberblox/, shipping its own server.ini
...
Zero real compat-shim gaps found. `console.right()` looked missing on
first grep (used in the high-scores column layout) but was a false
alarm -- it already existed in synchronet_compat.py (`right: function
(n){...}`, alongside the already-present left/up/down), the earlier
grep just searched for the wrong literal text. Confirmed live
end-to-end: the lobby/scoreboard rendered with real cross-BBS data
(properly column-aligned via console.right()) and a clean exit.

Follows the same create_app()/_create_default_data() pattern as
tests/test_druglord_door_seed.py.
"""
import os
import shutil
import sys
...
# Real vendored uberblox door files -- gitignored (third-party door
...
_DOOR_ROOT = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'uberblox'
...
@unittest.skipUnless(_DOOR_ROOT.is_dir(),
                    'requires the real vendored uberblox door files (gitignored)')
class UberbloxDoorSeedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_real_game_files_shipped_in_the_repo(self):
        ...
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'uberblox'
        self.assertTrue((root / 'blox.js').is_file())
        self.assertTrue((root / 'game.js').is_file())
        self.assertTrue((root / 'blox.bin').is_file())
        self.assertTrue((root / 'lobby.bin').is_file())
        self.assertTrue((root / 'gameend.bin').is_file())
        self.assertTrue((root / 'exit.bin').is_file())
        self.assertTrue((root / 'uberblox.bin').is_file())
    ...
    def test_seeded_row_has_correct_door_synchronet_config(self):
        ...
        with app.app_context():
            ...
            game = Game.query.filter_by(slug='uberblox').first()
    ...
    def test_seed_is_idempotent_on_a_second_boot(self):
        ...
        with app.app_context():
            ...
            _create_default_data()
            _create_default_data()
            rows = Game.query.filter_by(slug='uberblox').all()
