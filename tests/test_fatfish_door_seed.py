"""Regression test for the bundled "Fat Fish" door seed
(anetbbs/web_app.py, BUNDLED_DOORS list) -- the 8th door wired up to
...
Real "Art @ Fatcats BBS" door (a fishing simulation with real fish AI),
source unmodified, bundled at anetbbs/games/sbbs_doors/fatfish/,
shipping its own server.ini pointed at game.a-net-online.lol:10088
(StingRay's real live Synchronet JSON hub).
...
# Real vendored fatfish door files -- gitignored (third-party door
...
_DOOR_ROOT = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'fatfish'
...
@unittest.skipUnless(_DOOR_ROOT.is_dir(),
                    'requires the real vendored fatfish door files (gitignored)')
class FatfishDoorSeedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_real_game_files_shipped_in_the_repo(self):
        ...
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'fatfish'
        self.assertTrue((root / 'fatfish.js').is_file())
        self.assertTrue((root / 'ANSI.js').is_file())
        self.assertTrue((root / 'cast.js').is_file())
        self.assertTrue((root / 'Fish.js').is_file())
        self.assertTrue((root / 'Rod.js').is_file())
        self.assertTrue((root / 'safe_pause.js').is_file())
        self.assertTrue((root / 'shop.js').is_file())
        self.assertTrue((root / 'server.ini').is_file())
        self.assertTrue((root / 'title.ans').is_file())
        self.assertTrue((root / 'help.ans').is_file())
    ...
    def test_seeded_row_has_correct_door_synchronet_config(self):
        ...
        with app.app_context():
            ...
            game = Game.query.filter_by(slug='fatfish').first()
            ...
            self.assertTrue(game.synchronet_script_path.endswith(
                os.path.join('fatfish', 'fatfish.js')))
            ...
            # z-order fix (v1.0.36) for the gray shop-panel box.
    ...
    def test_seed_is_idempotent_on_a_second_boot(self):
        ...
        with app.app_context():
            ...
            _create_default_data()
            _create_default_data()
            rows = Game.query.filter_by(slug='fatfish').all()
