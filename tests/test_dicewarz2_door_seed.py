"""Regression test for the bundled "Dice Warz ][" door seed
(anetbbs/web_app.py, BUNDLED_DOORS list) -- the 9th door wired up to
...
Fat Fish. Real Matt Johnson / mcmlxxix door (a "Dice Wars"-style
territory-conquest strategy game), source unmodified, bundled at
anetbbs/games/sbbs_doors/dicewarz2/, shipping its own server.ini
pointed at game.a-net-online.lol:10088 (StingRay's real live
Synchronet JSON hub).
...
# Real vendored dicewarz2 door files -- gitignored (third-party door
...
_DOOR_ROOT = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'dicewarz2'
...
@unittest.skipUnless(_DOOR_ROOT.is_dir(),
                    'requires the real vendored dicewarz2 door files (gitignored)')
class Dicewarz2DoorSeedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_real_game_files_shipped_in_the_repo(self):
        ...
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'dicewarz2'
        self.assertTrue((root / 'dice2.js').is_file())
        self.assertTrue((root / 'game.js').is_file())
        self.assertTrue((root / 'dicefunc.js').is_file())
        self.assertTrue((root / 'diceobj.js').is_file())
        self.assertTrue((root / 'server.ini').is_file())
        self.assertTrue((root / 'dicewarz2.bin').is_file())

    def test_service_and_ai_js_are_not_bundled(self):
        """service.js (real server-side game/AI-turn authority) and
        ai.js (only load()'d BY service.js, never by the client) are
        real server-side-only files never load()'d by dice2.js's own
        client entry point -- matching the same exclusion already
        established for every other door's own commands.js/service.js/
        maintenance.js."""
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'dicewarz2'
        self.assertFalse((root / 'service.js').exists())
        self.assertFalse((root / 'ai.js').exists())
    ...
    def test_seeded_row_has_correct_door_synchronet_config(self):
        ...
        with app.app_context():
            ...
            game = Game.query.filter_by(slug='dicewarz2').first()
            ...
            self.assertTrue(game.synchronet_script_path.endswith(
                os.path.join('dicewarz2', 'dice2.js')))
            self.assertTrue(os.path.isfile(game.synchronet_script_path),
                            'seeded script path must actually resolve to a real file')
            # Confirmed working on real Pi3 hardware by Jerry, v1.0.38
            # (after 2 real bugs found from his playtest were fixed).
    ...
    def test_seed_is_idempotent_on_a_second_boot(self):
        ...
        with app.app_context():
            ...
            _create_default_data()
            _create_default_data()
            rows = Game.query.filter_by(slug='dicewarz2').all()
