"""Regression test for the bundled "Gooble Gooble" door seed
(anetbbs/web_app.py, BUNDLED_DOORS list) -- the 5th door wired up to
...
Boggle/Synchronetris/Jeopardized. Real echicken door (a Pac-Man clone),
source unmodified, bundled at anetbbs/games/sbbs_doors/gooble/, shipping
its own server.ini pointed at game.a-net-online.lol:10088 (StingRay's
...
# Real vendored gooble door files -- gitignored (third-party door
...
_DOOR_ROOT = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'gooble'
...
@unittest.skipUnless(_DOOR_ROOT.is_dir(),
                    'requires the real vendored gooble door files (gitignored)')
class GoobleDoorSeedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_real_game_files_shipped_in_the_repo(self):
        ...
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'gooble'
        self.assertTrue((root / 'gooble.js').is_file())
        self.assertTrue((root / 'server.ini').is_file())
        self.assertTrue((root / 'help.bin').is_file())
        self.assertTrue((root / 'splash.bin').is_file())
        self.assertTrue((root / 'sprites').is_dir())
        self.assertTrue((root / 'sprites' / 'gooble.bin').is_file())
        self.assertTrue((root / 'sprites' / 'gooble.ini').is_file())
        self.assertTrue((root / 'sprites' / 'ghost01.bin').is_file())

    def test_commands_and_service_js_are_not_bundled(self):
        """commands.js (json-service QUERY authorization hook) and
        service.js (server-side score-list aggregation) are real
        server-side-only files never load()'d by gooble.js's own
        client entry point -- matching the same exclusion already
        established for Jeopardized's commands.js/maintenance.js/
        service.js. Bundling them would be harmless but misleading
        (they'd never run client-side)."""
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'gooble'
    ...
    def test_seeded_row_has_correct_door_synchronet_config(self):
        ...
        with app.app_context():
            ...
            game = Game.query.filter_by(slug='gooble').first()
    ...
    def test_seed_is_idempotent_on_a_second_boot(self):
        ...
        with app.app_context():
            ...
            _create_default_data()
            _create_default_data()
            rows = Game.query.filter_by(slug='gooble').all()
