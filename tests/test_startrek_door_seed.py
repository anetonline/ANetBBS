"""Regression test for the bundled "Star Trek" door seed
(anetbbs/web_app.py, BUNDLED_DOORS list) -- the 7th door wired up to
...
Boggle/Synchronetris/Jeopardized/Gooble Gooble/Synkroban. Real echicken
door (a real-time space-combat arcade game), source unmodified, bundled
at anetbbs/games/sbbs_doors/startrek/, shipping its own server.ini
...
# Real vendored startrek door files -- gitignored (third-party door
...
_DOOR_ROOT = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'startrek'
...
@unittest.skipUnless(_DOOR_ROOT.is_dir(),
                    'requires the real vendored startrek door files (gitignored)')
class StartrekDoorSeedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_real_game_files_shipped_in_the_repo(self):
        ...
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'startrek'
        self.assertTrue((root / 'startrek.js').is_file())
        self.assertTrue((root / 'server.ini').is_file())
        self.assertTrue((root / 'help.bin').is_file())
        self.assertTrue((root / 'starfield.bin').is_file())
        self.assertTrue((root / 'startrek.bin').is_file())
        self.assertTrue((root / 'sprites').is_dir())
        self.assertTrue((root / 'sprites' / 'galaxy.bin').is_file())
        self.assertTrue((root / 'sprites' / 'galaxy.ini').is_file())
        self.assertTrue((root / 'sprites' / 'photonTorpedo1.bin').is_file())
    ...
    def test_seeded_row_has_correct_door_synchronet_config(self):
        ...
        with app.app_context():
            ...
            game = Game.query.filter_by(slug='startrek').first()
            ...
            self.assertTrue(game.synchronet_script_path.endswith(
                os.path.join('startrek', 'startrek.js')))
    ...
    def test_seed_is_idempotent_on_a_second_boot(self):
        ...
        with app.app_context():
            ...
            _create_default_data()
            _create_default_data()
            rows = Game.query.filter_by(slug='startrek').all()
