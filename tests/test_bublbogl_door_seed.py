"""Regression test for the bundled "Bubble Boggle" door seed
(anetbbs/web_app.py, BUNDLED_DOORS list) -- the second door wired up to
...
following the same shim contract proven by Chicken Delivery. Real The
BRoKEN BUBBLe Software (Matt Johnson) door, source unmodified, bundled
at anetbbs/games/sbbs_doors/bublbogl/, shipping its own server.ini
...
tests/test_chickendelivery_door_seed.py.
"""
import os
import shutil
import sys
...
# Real vendored bublbogl door files -- gitignored (third-party door
...
_DOOR_ROOT = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'bublbogl'
...
@unittest.skipUnless(_DOOR_ROOT.is_dir(),
                    'requires the real vendored bublbogl door files (gitignored)')
class BublboglDoorSeedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_real_game_files_shipped_in_the_repo(self):
        ...
        root = Path(__file__).resolve().parents[1] / 'anetbbs' / 'games' / 'sbbs_doors' / 'bublbogl'
        self.assertTrue((root / 'boggle.js').is_file())
        self.assertTrue((root / 'game.js').is_file())
        self.assertTrue((root / 'timer.js').is_file())
        self.assertTrue((root / 'server.ini').is_file())
        self.assertTrue((root / 'dict').is_dir())
        self.assertTrue((root / 'dict' / 'a.dic').is_file())
    ...
    def test_seeded_row_has_correct_door_synchronet_config(self):
        ...
        with app.app_context():
            ...
            game = Game.query.filter_by(slug='bublbogl').first()
            ...
            self.assertTrue(game.synchronet_script_path.endswith(
                os.path.join('bublbogl', 'boggle.js')))
    ...
    def test_seed_is_idempotent_on_a_second_boot(self):
        ...
        with app.app_context():
            ...
            _create_default_data()
            _create_default_data()
            rows = Game.query.filter_by(slug='bublbogl').all()
